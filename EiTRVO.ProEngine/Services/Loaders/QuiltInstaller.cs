...
internal static class QuiltInstaller
{
    ...
    {
        string url = QuiltLoaderVersions(mcVersion);
        string json = await http.GetStringAsync(url, ct);
        var entries = JsonSerializer.Deserialize<List<QuiltLoaderEntry>>(json)
                      ?? new List<QuiltLoaderEntry>();

        return entries
            .Where(e => e.Loader?.Version != null)
    ...
    {
        string profileUrl = QuiltLoaderProfile(mcVersion, loaderVersion);
        ...
        var profile = JsonSerializer.Deserialize<VersionDetail>(profileJson)
                      ?? throw new Exception("Quilt profile JSON 解析失败。");
        ...
        ModLoaderService.SaveInstanceMeta(versionDir, "Quilt", loaderVersion);
    }
}
